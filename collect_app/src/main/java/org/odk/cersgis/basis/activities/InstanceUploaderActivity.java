...
import org.odk.cersgis.basis.listeners.InstanceUploaderListener;
...
import org.odk.cersgis.basis.utilities.ArrayUtils;
...
public class InstanceUploaderActivity extends CollectAbstractActivity implements InstanceUploaderListener,
    ...
    @Override
    public void uploadingComplete(HashMap<String, String> result, String[] submittedForms) {
        ...
        if (!isInstanceStateSaved()) {
            try {
                triggerAggregator(submittedForms);
            } catch (Exception e) {
            }
    ...
    void triggerAggregator(String[] submittedForms) {
        ApiRequestAsync triggerRequestTask = new ApiRequestAsync(new ApiRequestListener() {
            @Override
        ...
        String forms = TextUtils.join(",", submittedForms);
        triggerRequestTask.downloadObject(Uris.getTriggerUrl() + phoneNumber + "?forms=[" + forms + "]");
    }
