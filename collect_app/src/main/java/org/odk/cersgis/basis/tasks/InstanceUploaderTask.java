        ...
        synchronized (this) {
            if (outcome != null && stateListener != null) {
                if (outcome.authRequestingServer != null) {
                        ...
                        try (Cursor results = new InstancesDao().getInstancesCursor(selection.toString(),
                            ...
                            if (results != null && results.getCount() > 0) {
                                ...
                                while (results.moveToNext()) {
                                    ...
                                    formVersion = results.getString(results.getColumnIndex(InstanceProviderAPI.InstanceColumns.JR_VERSION));
                                    String formName = results.getString(results.getColumnIndex(InstanceProviderAPI.InstanceColumns.DISPLAY_NAME));
                                    submittedList.add(formName);
                    ...
                    String[] submittedForms = new String[submittedList.size()];
                    submittedForms = submittedList.toArray(submittedForms);
                    stateListener.uploadingComplete(outcome.messagesByInstanceId, submittedForms);
                }
            }
        }
    }
